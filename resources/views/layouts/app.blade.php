        ...
        <div class="max-w-[1400px] mx-auto px-8 py-3 flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                @php
                    ...
                    if (Auth::check()) {
                        if (Auth::user()->can('staff')) {
                            $logoLink = route('attendance.top');
                        } elseif (Auth::user()->can('admin')) {
                            $logoLink = route('admin.staff.list');
                        }
                    }
                @endphp
                <a href="{{ $logoLink }}">
            ...
            @if (!Route::is('login') && !Route::is('register') && !Route::is('verification.notice'))
                <nav class="flex items-center gap-6 lg:gap-8 text-md font-bold lg:text-md flex-shrink-0">
                    @auth
                        @can('staff')
                            @if (session('is_completed') || (isset($attendance) && $attendance->status === '退勤済'))
                                <a href="{{ route('attendance.list') }}" class="hover:text-gray-300">今月の出勤一覧</a>
                                <a href="{{ route('stamp.list') }}" class="hover:text-gray-300">申請一覧</a>
                            @else
                        ...
                        @elsecan('admin')
                            ...
                            <a href="{{ route('admin.stamp.approve', ['attendance_correct_request_id' => 'list']) }}"
                                class="hover:text-gray-300">申請一覧</a>
                        @endcan
